...
#
##We ran the model twice once with a higher learning rate and lower number of iterations and again with a lower learning rate and higher number 
##of iterations. In the first instance, the test mse reduces uptill iteration 10 and then starts increasing. While in second case, the mse keeps
##reducing uniformly until the last iteration. Hence it is generally better to use less learning rate and high number of iterations. 
#
...
#
##Here we see that the performance of learning rate 0.001 and 0.01 are almost similar, but still we would choose 0.001 because it converges 
##evenly with the number of iterations.
#        
#iter_array = [1, 2, 5, 10, 25, 50, 100, 200]
...
#            print (pd.Series(best_params))
#            
##The GridSearch results show that for 100 iterations almost all values of regularization params/latent factors produce similar MSEs. But
##since we chose 0.001 learning rate, we will select that, and number of factors can be taken as 40. Not too high but not lower since that
##might be vulnerable to underfitting. 
##Learning Rate - 0.001
...
##Final model fitting
#
#MF_SGD = ExplicitMF(train_data_matrix, 40, learning='sgd',\
#                            user_fact_reg=0.001, item_fact_reg=0.001, \
#                            user_bias_reg=0.001, item_bias_reg=0.001, verbose=True)
#iter_array = [1, 2, 5, 10, 25, 50, 100, 200]
#MF_SGD.calculate_learning_curve(iter_array, test_data_matrix, learning_rate=0.001)
#
#import pickle
...
#pickle.dump(MF_SGD, open(filename, 'wb'))

if __name__ == "__main__":
    sgd = ExplicitMF(train_data_matrix, 40, learning='sgd',\
                           user_fact_reg=0.001, item_fact_reg=0.001, \
                           user_bias_reg=0.001, item_bias_reg=0.001, verbose=True)
    sgd.save("book-recommending-sgd-model.pkl")